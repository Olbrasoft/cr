...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/pamatky/", axum::routing::get(handlers::landmarks_index))
        .route("/hrady", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("castle".to_string()), q)))
        .route("/hrady/", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("castle".to_string()), q)))
        .route("/zamky", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("chateau".to_string()), q)))
        .route("/zamky/", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("chateau".to_string()), q)))
        .route("/rozhledny", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("lookout_tower".to_string()), q)))
        .route("/rozhledny/", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("lookout_tower".to_string()), q)))
        .route("/vodni-mlyny", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("watermill".to_string()), q)))
        .route("/vodni-mlyny/", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("watermill".to_string()), q)))
        .route("/vetrne-mlyny", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("windmill".to_string()), q)))
        .route("/vetrne-mlyny/", axum::routing::get(|s, q| handlers::landmarks_by_type(s, axum::extract::Path("windmill".to_string()), q)))
