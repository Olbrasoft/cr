...
- **Draft PRs are NOT reviewed by Copilot.** Copilot skips draft PRs entirely — no `code-review-complete` wake will ever arrive while the PR is in draft state. If you want a review, you MUST run `gh pr ready <pr>` to mark it as ready-for-review. Do NOT sit waiting for a review wake on a draft; either mark it ready (to get a review) or merge it directly (if intentionally skipping review).
