    ...
    match segments.len() {
        1 => {
            ...
            if orp_check.is_some() {
                return Ok(orp::render_orp_by_slug(&state, segments[0]).await.into_response());
            }
            Ok(regions::render_region(&state, segments[0]).await.into_response())
        }
        2 => {
            ...
            if orp_check.is_some() {
                ...
                let result = municipalities::render_municipality_short(&state, segments[0], segments[1]).await;
                if result.0 == StatusCode::NOT_FOUND {
                    let landmark_result = landmarks::render_landmark_short(&state, segments[0], segments[1]).await;
                    if landmark_result.0 == StatusCode::NOT_FOUND {
                        return Ok(pools::render_pool_short(&state, segments[0], segments[1]).await.into_response());
                    }
        ...
        3 => {
            ...
            if let Some(oid) = orp_id {
                ...
                if muni_id.is_some() {
                    ...
                    let landmark_result = landmarks::render_landmark_in_municipality(&state, segments[0], segments[2], oid).await;
                    if landmark_result.0 != StatusCode::NOT_FOUND {
                        return Ok(landmark_result.into_response());
                    }
                    return Ok(pools::render_pool_short(&state, segments[0], segments[2]).await.into_response());
                }
            }
