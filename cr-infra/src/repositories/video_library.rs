...
//! `migrations/20260412_021_create_videos.sql`. The unique constraint
//! `(source_url, quality)` is the dedup key — `find_by_source_and_quality`
//! is the cheap path the upload pipeline calls before invoking yt-dlp.
    ...
    async fn insert(&self, video: NewVideo) -> Result<i32, Self::Error> {
        let id: i32 = sqlx::query_scalar(
            "INSERT INTO videos (\
                ...
                file_size_bytes, thumbnail_r2_key, thumbnail_url\
    ...
    async fn find_by_source_and_quality(
        ...
        let sql = format!(
            "SELECT {SELECT_COLUMNS} FROM videos \
             WHERE source_url = $1 AND quality = $2"
        );
