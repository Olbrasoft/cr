...
def get_orp_slugs():
    """Get ORP code → slug mapping from database."""
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()
    cur.execute("SELECT orp_code, slug FROM orp")
...
def generate_region_svg(features, region_slug, orp_slugs, svg_width=480):
    ...
    for feat in features:
        ...
        orp_slug = orp_slugs.get(orp_code, orp_name.lower().replace(" ", "-"))
...
def main():
    ...
    for nuts3, features in sorted(by_region.items()):
        ...
        svg = generate_region_svg(features, region_slug, orp_slugs)
